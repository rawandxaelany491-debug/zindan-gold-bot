...
if GEMINI_API_KEY:
    try:
        ...
        print("✅ Gemini پێکهێنرا!")
    except Exception as e:
        print(f"❌ هەڵە لە پێکهێنانی Gemini: {e}")
else:
    print("⚠️ کلیلی Gemini نەدۆزرایەوە. شیکاری وێنە ناکارێت.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not GEMINI_AVAILABLE:
        await update.message.reply_text("⚠️ ببورە، کلیلی Gemini دیاری نەکراوە. تکایە GEMINI_API_KEY لە .env دابنێ.")
        return
    ...
    try:
        ...
        logging.error(f"هەڵەی Gemini: {e}")
...
def main():
    if not BOT_TOKEN:
        print("❌ تۆکنی بۆت نەدۆزرایەوە! لە .envدا BOT_TOKEN دابنێ.")
        return
    ...
    # زیادکردنی مامەڵەکەر بۆ وێنە (ئەمە لە پێشینەیە، پێویستە لەسەر MessageHandler دابنرێت)
    app.add_handler(MessageHandler(filters.PHOTO, handle_photo))
    
    # زیادکردنی مامەڵەکەر بۆ تێکست (دوای هەموو فەرمانەکان و وێنەکان)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, echo))

    print("✅ بۆتی SNRZ بە Gemini کاردەکات! چاوەڕێی پەیام بن.")
    app.run_polling()
